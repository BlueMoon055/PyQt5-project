...
def delete_1_element():
    current_item = list_left.currentItem()  # получаем текущий выбранный элемент
    if current_item:  # проверяем, что элемент выбран (не None)
        delete_from_db(current_item.text(), 0) # в скобках аргументы
...
def delete_1_element_done():
    current_item = list_right.currentItem()  # получаем текущий выбранный элемент
    if current_item:  # проверяем, что элемент выбран (не None)
        delete_1_element_db_done(current_item.text(), 1) # в скобках аргументы
        list_right.takeItem(list_right.currentRow()) # удаляем выбранный элемент
        list_right.setCurrentRow(-1) # выделить строку под номером -1

app = QApplication([]) # создание объекта-приложения
app.setWindowIcon(QIcon(resource_path("icon.ico"))) # устанавливаем иконку для приложения
...
palette.setBrush(main_win.backgroundRole(), QBrush(QPixmap(resource_path('фон.png')))) # устанавливаем фон
main_win.setPalette(palette) # применяем изменённую палитру к окну
# устанавливаем розовый цвет для виджетов
